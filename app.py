...
if page == "📊 Dashboard":
    ...
    with col2:
        ...
        df = run_query("SELECT Claim_Day, COUNT(*) AS claims FROM claims GROUP BY Claim_Day ORDER BY claims DESC")
    ...
    df = run_query("SELECT Expiry_Status, COUNT(*) AS count, SUM(Quantity) AS total_qty FROM food_listings GROUP BY Expiry_Status")
...
# ═══════════════════════════════════════════════════════════════
    ...
    sql = f"""
        SELECT f.Food_ID, f.Food_Name, f.Quantity, f.Expiry_Date,
               ...
               f.Expiry_Status, f.Days_Until_Expiry, p.Name AS Provider_Name
    ...
    if exp_status != "All":
        sql += " AND f.Expiry_Status = %s"; params.append(exp_status)
...
# ═══════════════════════════════════════════════════════════════
    ...
    queries = {
        ...
        "Q8 — Unclaimed food listings": "SELECT f.Food_ID, f.Food_Name, f.Quantity, f.Expiry_Date, f.Expiry_Status, f.Days_Until_Expiry, f.Food_Type, f.Meal_Type, p.Name AS provider_name, p.City FROM food_listings f JOIN providers p ON f.Provider_ID=p.Provider_ID LEFT JOIN claims c ON f.Food_ID=c.Food_ID WHERE c.Claim_ID IS NULL ORDER BY f.Days_Until_Expiry ASC LIMIT 20",
        ...
        "Q10 — Daily claim activity trend": "SELECT Claim_Day, COUNT(*) AS total_claims, SUM(CASE WHEN Status='Completed' THEN 1 ELSE 0 END) AS completed, SUM(CASE WHEN Status='Cancelled' THEN 1 ELSE 0 END) AS cancelled, SUM(CASE WHEN Status='Pending' THEN 1 ELSE 0 END) AS pending, ROUND(SUM(CASE WHEN Status='Completed' THEN 1 ELSE 0 END)*100.0/COUNT(*),2) AS completion_rate_pct FROM claims GROUP BY Claim_Day ORDER BY total_claims DESC",
        "Q11 — Expiry urgency vs claim status": "SELECT f.Expiry_Status, COUNT(DISTINCT f.Food_ID) AS food_listings, COUNT(c.Claim_ID) AS total_claims, SUM(CASE WHEN c.Status='Completed' THEN 1 ELSE 0 END) AS completed, SUM(CASE WHEN c.Status='Cancelled' THEN 1 ELSE 0 END) AS cancelled, SUM(CASE WHEN c.Status='Pending' THEN 1 ELSE 0 END) AS pending, ROUND(SUM(CASE WHEN c.Status='Completed' THEN 1 ELSE 0 END)*100.0/NULLIF(COUNT(c.Claim_ID),0),2) AS completion_rate_pct FROM food_listings f LEFT JOIN claims c ON f.Food_ID=c.Food_ID GROUP BY f.Expiry_Status",
